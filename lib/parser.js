...
module.exports=function(tagname,parent,val){this.tagname=tagname;this.parent=parent;this.child=[];//child tags
this.attrsMap={};//attributes map
this.cdata=false;this.val=val;//text only
...
}var xmlObj=new(options.arrayMode==="children"?xmlNode2:xmlNode)('!xml');var currentNode=xmlObj;var tagsRegx=new RegExp(regx,'g');var tag=tagsRegx.exec(xmlData);var nextTag=tagsRegx.exec(xmlData);while(tag){var tagType=checkForTagType(tag);if(tagType===TagType.CLOSING){//add parsed data to parent node
