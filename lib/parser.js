...
module.exports=function(tagname,parent,val){this.tagname=tagname;this.parent=parent;this.child=[];//child tags
this.attrsMap={};//attributes map
this.cdata=0;this.val=val;//text only
this.addChild=function(child){this.child.push(child);};};
...
"use strict";
var util=__webpack_require__(/*! ./util */ "./src/util.js");var buildOptions=__webpack_require__(/*! ./util */ "./src/util.js").buildOptions;var xmlNode=__webpack_require__(/*! ./xmlNode */ "./src/xmlNode.js");var xmlNode2=__webpack_require__(/*! ./xmlNode2 */ "./src/xmlNode2.js");var TagType={OPENING:1,CLOSING:2,SELF:3,CDATA:4};var regx='<((!\\[CDATA\\[([\\s\\S]*?)(]]>))|((NAME:)?(NAME))([^>]*)>|((\\/)(NAME)\\s*>))([^<]*)'.replace(/NAME/g,util.nameRegexp);var xmlRegx='^<\\?xml[\\s\\S]*?\\?>';//const tagsRegx = new RegExp("<(\\/?[\\w:\\-\._]+)([^>]*)>(\\s*"+cdataRegx+")*([^<]+)?","g");
...
parseNodeValue:true,parseAttributeValue:false,parseAnnotation:false,arrayMode:false,// true, children, strict
...
};exports.defaultOptions=defaultOptions;var props=['attributeNamePrefix','attrNodeName','textNodeName','ignoreAttributes','ignoreNameSpace','allowBooleanAttributes','parseNodeValue','parseAnnotation','parseAttributeValue','arrayMode','trimValues','cdataTagName','cdataPositionChar','tagValueProcessor','attrValueProcessor','parseTrueNumberOnly','stopNodes'];exports.props=props;var getTraversalObj=function getTraversalObj(xmlData,options){options=buildOptions(options,defaultOptions,props);//xmlData = xmlData.replace(/\r?\n/g, " ");//make it single line
if(options.parseAnnotation){xmlData=xmlData.replace(/<!--([\s\S]*?)-->/g,'<__annotation><![CDATA[$1]]></__annotation>');// 支持注释
}else{xmlData=xmlData.replace(/<!--[\s\S]*?-->/g,'');//Remove  comments
}var xmlTagRegx=new RegExp(xmlRegx);var xmlObj=new(options.arrayMode==="children"?xmlNode2:xmlNode)('!xml');;var xmlTag=xmlTagRegx.exec(xmlData);if(xmlTag&&xmlTag.length>0){xmlObj.attrsMap=buildAttributesMap(xmlTag[0],options);}var currentNode=xmlObj;var tagsRegx=new RegExp(regx,'g');var tag=tagsRegx.exec(xmlData);var nextTag=tagsRegx.exec(xmlData);while(tag){var tagType=checkForTagType(tag);if(tagType===TagType.CLOSING){//add parsed data to parent node
